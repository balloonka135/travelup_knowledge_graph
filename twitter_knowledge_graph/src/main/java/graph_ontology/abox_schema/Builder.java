...
public class Builder {
    ...
    public void build_tweet(Model model) throws IOException {
        ...
        BufferedReader csvReader = reader.open(conf.TWEET_PATH);
        String row;
        while ((row = csvReader.readLine()) != null) {
            String[] row_data = row.split(",");

            String tweetURI = row_data[0];
            String tweetContent = row_data[1];
            String tweetPublishedDate = row_data[2];
    ...
    public void build_has_location(Model model) throws IOException {
        ...
        BufferedReader csvReader =  reader.open(conf.HAS_LOCATION_PATH);
        String row;
        while ((row = csvReader.readLine()) != null) {
            String[] row_data = row.split(",");
